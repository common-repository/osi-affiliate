...
$osi_hss_snippetId = $_GET['snippetId'];
...
if (isset($_POST) && isset($_POST['updateSubmit'])) {
    ...
    $osi_hss_snippetId = esc_sql( $_GET['snippetId'] );
    ...
    if ($osi_hss_title != "" && $osi_hss_content != "") {
        ...
        if (ctype_alnum($temp_osi_hss_title)) {
            $snippet_count = $wpdb->query($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'osi_hss_short_code WHERE id!=%d AND title=%s LIMIT 0,1', $osi_hss_snippetId, $osi_hss_title));

            if ($snippet_count == 0) {
                $osi_shortCode = '[osi-hss snippet="' . $osi_hss_title . '"]';

                $wpdb->update($wpdb->prefix . 'osi_hss_short_code', array('title' => $osi_hss_title, 'content' => $osi_hss_content, 'short_code' => $osi_shortCode,), array('id' => $osi_hss_snippetId));

                osi_js_redirect('page=html-snippet-shortcode-manage&msg=5');
            } else {
                ?>
...
$snippetDetails = $wpdb->get_results($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'osi_hss_short_code WHERE id=%d LIMIT 0,1', $osi_hss_snippetId));
    ...
    <fieldset
        ...
        <form name="frmmainForm" id="frmmainForm" method="post">
            <input type="hidden" id="snippetId" name="snippetId"
                   value="<?php if (isset($_POST['snippetId'])) {
    echo esc_attr($_POST['snippetId']);
} else {
    echo esc_attr($snippetDetails->id);
} ?>">
            <div>
                <table
                    ...
                    <tr valign="top">
                        ...
                        <td><input style="width:80%;"
                                   ...
                                   value="<?php if (isset($_POST['snippetTitle'])) {
    echo esc_attr($_POST['snippetTitle']);
} else {
    echo esc_attr($snippetDetails->title);
} ?>"></td>
                    </tr>
                    <tr>
                        ...
                        <td style="border-bottom: none;width:1px;">&nbsp;:&nbsp;</td>
                        <td >
                            <textarea name="snippetContent" style="width:80%;height:150px;"><?php if (isset($_POST['snippetContent'])) {
    echo esc_textarea($_POST['snippetContent']);
} else {
    echo esc_textarea($snippetDetails->content);
} ?></textarea>
                        </td>
                    </tr>				
